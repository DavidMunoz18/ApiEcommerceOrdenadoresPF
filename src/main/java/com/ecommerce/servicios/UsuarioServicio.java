...
public class UsuarioServicio {
    ...
    /**
     * Realiza el proceso de login con Google.
     * Busca si el usuario ya existe basándose en su email.
     * - Si no existe, lo registra.
     * - Si existe, retorna el usuario ya registrado.
     * 
     * @param usuarioDto El DTO con los datos del usuario obtenido desde el login con Google.
     * @return El DTO del usuario autenticado o registrado.
     */
    public UsuarioDto loginGoogle(UsuarioDto usuarioDto) {
        // Buscar usuario por email
        UsuarioDao usuarioExistente = usuarioRepository.findByEmailUsuario(usuarioDto.getEmailUsuario());
        ...
        if (usuarioExistente == null) {
            ...
            nuevoUsuario.setEmailUsuario(usuarioDto.getEmailUsuario());
            nuevoUsuario.setPasswordUsuario(usuarioDto.getContrasena()); // "google_autogenerated" u otro valor
            nuevoUsuario.setEsGoogle(usuarioDto.isEsGoogle());
            // Asignar rol por defecto
            nuevoUsuario.setRol("usuario");
            
            // Se puede asignar otros valores por defecto si se requiere.
            nuevoUsuario = usuarioRepository.save(nuevoUsuario);
            
            // Convertir la entidad a DTO para retornar
            return convertirADto(nuevoUsuario);
        } else {
            // Si el usuario ya existe, retornarlo convertido a DTO
            return convertirADto(usuarioExistente);
        }
    }
    
    /**
     * Método helper que convierte la entidad UsuarioDao a UsuarioDto.
     * 
    ...
    private UsuarioDto convertirADto(UsuarioDao usuario) {
        ...
        dto.setContrasena(usuario.getPasswordUsuario());
        dto.setRol(usuario.getRol());
        dto.setEsGoogle(usuario.isEsGoogle());
        // Omitimos campos como foto y token según necesidad.
        return dto;
    }
